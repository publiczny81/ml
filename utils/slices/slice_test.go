...
func TestInitiate(t *testing.T) {
	var (
		tests = []struct {
		...
		}{
			{
				Name:    "f",
				Initial: make([]int, 3),
				InitFunc: func(idx int, value int) int {
					return idx + value
				},
				Expected: []int{0, 1, 2},
			},
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			var actual = Initiate(test.Initial, test.InitFunc)
...
func TestReverse(t *testing.T) {
	var (
		tests = []struct {
			...
			{
				Name:   "When function returns false then it stops iterating",
				Actual: []*int{utils.Pointer(1), utils.Pointer(2), utils.Pointer(3)},
				Function: func(i *int) bool {
					if *i < 2 {
						return false
					}
				...
				Expected: []*int{utils.Pointer(1), utils.Pointer(0), utils.Pointer(0)},
			},
		}
	)
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			Reverse(test.Actual, test.Function)
			assert.Equal(t, test.Expected, test.Actual)
		})
	}
}
...
func TestReverseWithIndex(t *testing.T) {
	var (
		tests = []struct {
			Name     string
			Actual   []*int
			...
			},
			{
				...
				Expected: []*int{utils.Pointer(0), utils.Pointer(1), utils.Pointer(1)},
			},
		}
	)
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			ReverseWithIndex(test.Actual, test.Function)
			assert.Equal(t, test.Expected, test.Actual)
		})
	}
}
